...
export function calculateNextExecution(
  ...
  if (schedule.frequency === "hourly") {
    ...
    const lastExecutedUTC = schedule.last_executed
  ...
  const lastExecutedInZone = schedule.last_executed
    ? toZonedTime(new Date(schedule.last_executed), timezone)
    : null;
  ...
  candidateDate = set(candidateDate, {
    hours,
    minutes,
    seconds: 0,
    milliseconds: 0,
  ...
  const startDate = schedule.start_date
    ? toZonedTime(new Date(schedule.start_date), timezone)
    : null;
  const endDate = schedule.end_date
    ? toZonedTime(new Date(schedule.end_date), timezone)
    : null;

  if (startDate && candidateDate < startDate) {
    candidateDate = set(startDate, {
      hours,
      minutes,
      seconds: 0,
      milliseconds: 0,
  ...
  for (let i = 0; i < 730; i++) {
    ...
    switch (schedule.frequency) {
      ...
      case "weekly":
        const desiredDays =
          schedule.selected_weekdays?.map(
      ...
      case "yearly":
        isValidDay =
          ...
          (!schedule.selected_year ||
            candidateDate.getFullYear() === schedule.selected_year);
        break;
      case "specific_dates":
        const formattedCandidate = format(candidateDate, "yyyy-MM-dd", {
          timeZone: timezone,
        });
        isValidDay =
          schedule.specific_dates?.includes(formattedCandidate) ?? false;
        break;
    }
