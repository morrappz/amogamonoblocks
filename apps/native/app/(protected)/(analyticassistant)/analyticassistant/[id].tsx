...
const AnalyticAssistant = () => {
    ...
    onFinish: async (message, { finishReason }) => {
      ...
      if (message && finishReason === "stop") {
        ...
        const messagesToSave = [{
          ...
          chatId: (newChatIdRef.current || chatId?.id || newChatId)
        }]
  ...
  const [activePopover, setActivePopover] = useState<'history' | 'bookmarks' | 'favorites' | 'tokens' | null>(null);
  ...
  const handlePopoverOpen = (type: 'history' | 'bookmarks' | 'favorites' | 'tokens') => {
  ...
  const handleAudioSubmit = (audioBase64: string, mimeType: string) => {
    const submissionChatId = (newChatIdRef.current || chatId?.id || newChatId);
