							...
							<Card className="bg-primary/10 border border-primary rounded-xl p-4">
								<CardContent className="p-0">
									<H4 className="text-primary mb-1">
										👋 Welcome back, {userCatalog?.first_name}!
									</H4>
									<Text className="text-sm leading-tight">
										Here's a quick overview of your dashboard. Explore your
										pages or manage your content below.
									</Text>
								</CardContent>
							</Card>
						</View>
								...
								{filteredPages.map((page, index) => (
									<View key={index} className="w-1/2 md:w-1/3 px-1 mb-3">
										<PressableCard
											...
											onPress={() => console.log(`Pressed ${page.page_name}`)}
										>
											<CardContent className="flex-1 p-0 justify-between items-center">
												{page.page_icon_name ? (
													<Text>
														<LucideIcon
															name={page.page_icon_name as any}
															size={32}
												...
												<P className="text-center text-xs leading-tight line-clamp-2">
													description
												</P>
											</CardContent>
										</PressableCard>
